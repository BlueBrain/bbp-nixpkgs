  ...
  common =
    ...
    stdenv.mkDerivation rec {
      ...
      installPhase = ''
        ...
        mv "$out"/samples "$doc"/samples

        # Change the #error on GCC > 4.9 to a #warning.
        sed -i $out/include/host_config.h -e 's/#error\(.*unsupported GNU version\)/#warning\1/'

       # separate cuda C/C++ and java tools
       mv $out/jre $java/jre || true
       mv $out/nsightee_plugins $java/nsightee_plugins || true
       mv $out/libnvvp $java/libnvvp || true   
       mv $out/libnsight $java/libnsight || true
